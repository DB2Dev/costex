...
def query_builder():
    ...
    data = {
        "EMPLOYEE": [att[1] for row in Constraints.get_columns(TableType.EMPLOYEE)],
        "PROJECT": [att[1] for row in Constraints.get_columns(TableType.PROJECT)],
    }
    ...
    if select_all_columns:
        ...
        selected_columns = st.multiselect(f'Select Columns from {selected_table}',data[selected_table])
    ...
    if join_checkbox:
        ...
        condition_col1 = row1_columns[0].selectbox(f'Select Column from {selected_table}', data[selected_table])
        condition_sign = row1_columns[1].selectbox('Equal Sign', ['='])
        condition_col2 = row1_columns[2].selectbox(f'Select Column from {join_table}', data[join_table], key='join_condition_col2')
    ...
    condition_col = row1_columns[0].selectbox(f'Select Column from {selected_table}', data[selected_table], key='where_condition_col')
    ...
    while st.checkbox(f'EXTRA CONDITION', key=f'extra_condition_{i}'):
        ...
        condition_col = row1_columns[1].selectbox('Column',data[selected_table], key=f'extra_condition_col_{i}')
        ...
        if condition_logic == 'OR':
            if condition_sign != 'BETWEEN':
                ...
                where_clause += f") {condition_logic} ({selected_table}.{condition_col} BETWEEN '{condition_value1}' AND '{condition_value2}'"
        else:
            if condition_sign != 'BETWEEN':
                condition_value = row1_columns[3].text_input('Value', '', key=f'extra_condition_value_{i}')
                where_clause += f" {condition_logic} {selected_table}.{condition_col} {condition_sign} '{condition_value}'"
        ...
        if st.checkbox(f'EXTRA CONDITION', key=f'extra_condition_{i}'):
            
            
            conditions.append(Condition(
                column_name=condition_col,
                operator= filter_operators[condition_sign],
                condition_type=QueryType.EQUALITY if condition_sign == '=' else QueryType.RANGE,
                values=[condition_value1, condition_value2] if condition_sign == 'BETWEEN' else [condition_value]
            ))
            logical_operators.append(LogicalOperator.OR if condition_logic == 'OR' else LogicalOperator.AND)
        else:
            # Remove the condition from the list if the checkbox is unchecked
            conditions = [cond for cond in conditions if cond.column_name != condition_col]
            logical_operators = [op for op in logical_operators if op != LogicalOperator.OR ]if condition_logic == 'OR' else [op for op in logical_operators if op != LogicalOperator.AND]
        i += 1
    ...
    if st.button('Generate Query Report'):
        ...
        query = Query(
            operation=QueryOperation.SELECT if (join_checkbox == False) else QueryOperation.JOIN,
            table_name= TableType.EMPLOYEE if (selected_table == "EMPLOYEE") else TableType.PROJECT,
            select_columns_names=selected_columns,
            filters=conditions,
            filters_operators=logical_operators,
            join_column_names= join_col_names if (join_checkbox == True) else None,
            join_table_name= TableType.EMPLOYEE if (join_table == "EMPLOYEE") else TableType.PROJECT,
        )
