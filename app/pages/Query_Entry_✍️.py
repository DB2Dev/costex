...
def query_builder():
    ...
    if join_checkbox:
        ...
        condition_col1 = st.selectbox(f'Select Column from {selected_table}', ['col1', 'col2', 'col3'])
        condition_sign = st.selectbox('Equal Sign', ['='])
        condition_col2 = st.selectbox(f'Select Column from {join_table}', ['col1', 'col2', 'col3'], key='join_condition_col2')
    ...
    # WHERE clause
    st.subheader('WHERE')
    condition_col = st.selectbox(f'Select Column from {selected_table}', ['col1', 'col2', 'col3'], key='where_condition_col')
    condition_sign = st.selectbox('Select Sign', ['=', '>', '<', '>=', '<='], key='where_condition_sign')
    condition_value = st.text_input('Enter Value', '', key='where_condition_value')
    ...
    while st.checkbox(f'EXTRA CONDITION', key=f'extra_condition_{i}'):
        condition_col = st.selectbox(f'Select Column from {selected_table}', ['col1', 'col2', 'col3'], key=f'extra_condition_col_{i}')
        condition_sign = st.selectbox('Select Sign', ['=', '>', '<', '>=', '<='], key=f'extra_condition_sign_{i}')
        condition_value = st.text_input('Enter Value', '', key=f'extra_condition_value_{i}')
        
        condition_logic = st.radio(f'Select Condition Logic for Condition {i}:', ['AND', 'OR'], key=f'condition_logic_{i}')
